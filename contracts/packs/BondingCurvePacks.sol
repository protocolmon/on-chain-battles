...
struct BondingCurve {
    uint256 factor; // 1 = 1e18
    uint256 exponent;
    uint256 c; // 1 = 1e18
    uint256 maxSupply;
}

contract BondingCurveMons is
    Initializable,
    ERC721AUpgradeable,
    OwnableUpgradeable,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    BondingCurve public bondingCurveSpecs;
    ...
    function initialize(
        ...
        BondingCurve memory args_bondingCurveSpecs,
        IBlast args_blast,
        ...
        bondingCurveSpecs = args_bondingCurveSpecs;
        blast = args_blast;
        ...
        nonReentrant // since we send eth we go safe and use nonReentrant. @todo: check the extra gas
    {
        ...
        require(
            totalSupply() + quantity <= bondingCurveSpecs.maxSupply,
            "Max supply reached"
        );

        // get the price & check if enough value
        uint256 price = getPrice(totalSupply(), quantity);
        uint256 protocolFee = (price * FEE) / (1 ether); // @todo: is that precision actually necessary ?
        require(msg.value >= price + protocolFee, "Value too low");

        // mint
        _mint(to, quantity);

        // send fees out to fee receiver and excess value back to msg.sender
        Address.sendValue(feeReceiver, protocolFee);
        Address.sendValue(payable(msg.sender), msg.value - price - protocolFee);
        // @todo: check if we should add an assert here

        emit Purchase(msg.sender, to, quantity, price, protocolFee);
    }

    /** selling */

    function sellToBondingCurve(
        ...
        nonReentrant // since we send eth we go safe and use nonReentrant. @todo: check the extra gas
    {
        ...
        require(amount > 0, "Amount has to be larget than 0");
        require(block.timestamp <= deadline, "Deadline passed");

        // get the price and fees
        uint256 price = getPrice(totalSupply() - amount, amount);
        uint256 protocolFee = (price * FEE) / (1 ether); // @todo: is that precision actually necessary ?
        require(price - protocolFee >= minPrice, "min price not reached");
        ...
        emit Sale(msg.sender, receiver, amount, price, protocolFee);
    }
    ...
    function getPrice(
        ...
        uint256 factor = bondingCurveSpecs.factor;
        uint256 exponent = bondingCurveSpecs.exponent;
        uint256 c = bondingCurveSpecs.c;

        uint256 lower = getIntegral(factor, supply, exponent, c);
        uint256 upper = getIntegral(factor, supply + amount, exponent, c);
    ...
    function getBuyPriceInclFees(
        ...
        uint256 price = getPrice(totalSupply(), args_amount);
        // protocolFee = (price * FEE) / (1 ether);
        // buyPriceInclFees = price + protocolFee;
        return price + (price * FEE) / (1 ether);
    }

    function getSellPriceInclFees(
        uint256 args_amount
    ) public view returns (uint256) {
        uint256 price = getPrice(totalSupply() - args_amount, args_amount);
        // protocolFee = (price * FEE) / (1 ether);
        // sellPriceInclFees = price - protocolFee;
        return price - (price * FEE) / (1 ether);
    }
    ...
    function getIntegral(
        ...
        uint256 exponent,
        uint256 c // constant
    ) public pure returns (uint256 integral) {
        return
            (supply * ((factor * supply ** exponent) / (exponent + 1) + c)) /
            (1 ether);
    }
